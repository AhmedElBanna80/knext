 ...
 *                via ARGV (never via shell string — no injection risk)
 *
...
// ---------------------------------------------------------------------------
// validateTaggedRef — defense-in-depth: reject refs with shell metacharacters
// ---------------------------------------------------------------------------

describe("validateTaggedRef", () => {
    it("accepts a normal registry/name:tag ref", () => {
        expect(() =>
            validateTaggedRef("registry.example.com/my-app:20240101"),
        ).not.toThrow();
    });
    ...
    it("accepts a digest-pinned ref", () => {
        expect(() =>
            validateTaggedRef(
                `registry.example.com/my-app:v1@sha256:${FAKE_DIGEST.slice(7)}`,
            ),
        ).not.toThrow();
    });

    it("rejects a ref containing semicolon (shell command separator)", () => {
        expect(() =>
            validateTaggedRef("registry.example.com/my-app:tag;touch x"),
        ).toThrow(/invalid/i);
    });

    it("rejects a ref containing backtick (shell command substitution)", () => {
        expect(() =>
            validateTaggedRef("registry.example.com/my-app:tag`id`"),
        ).toThrow(/invalid/i);
    });

    it("rejects a ref containing dollar-paren (shell command substitution)", () => {
        expect(() =>
            validateTaggedRef("registry.example.com/my-app:tag$(id)"),
        ).toThrow(/invalid/i);
    });

    it("rejects a ref containing space", () => {
        expect(() =>
            validateTaggedRef("registry.example.com/my-app:tag extra"),
        ).toThrow(/invalid/i);
    });

    it("rejects a ref containing newline", () => {
        expect(() =>
            validateTaggedRef("registry.example.com/my-app:tag\necho hi"),
        ).toThrow(/invalid/i);
    });
});
...
describe("resolveDigest — docker-inspect fallback (FALLBACK path)", () => {
    it("falls back to docker inspect when metadataFilePath is not provided", async () => {
        ...
        // The spy MUST receive a string[] — ARGV, never a shell string.
        const argv = execSpy.mock.calls[0][0] as string[];
        expect(Array.isArray(argv)).toBe(true);

        // argv[0] must be the binary name, not a shell string like "sh"
        expect(argv[0]).toBe("docker");
        expect(argv[1]).toBe("inspect");

        // taggedRef must appear as a SINGLE, LAST element — no shell concatenation.
        expect(argv[argv.length - 1]).toBe(taggedRef);

        expect(result).toContain("@sha256:");
    });

    it("exec spy receives taggedRef as a standalone array element (injection-safety proof)", async () => {
        ...
        // execFn must NEVER have been called with the unsafe ref
        expect(execSpy).not.toHaveBeenCalled();
    });
    ...
    it("falls back to docker inspect when metadata file read throws", async () => {
        ...
        // Fell back to docker inspect — and argv must be an array
        expect(execSpy).toHaveBeenCalledTimes(1);
        const argv = execSpy.mock.calls[0][0] as string[];
        expect(Array.isArray(argv)).toBe(true);
        expect(result).toContain("@sha256:");
    ...
    it("throws when both metadata and docker inspect fail", async () => {
        ...
        await expect(
            resolveDigest(
                "registry.example.com/my-app:bad",
                execSpy,
                "/tmp/meta.json",
                readFileFn,
            ),
