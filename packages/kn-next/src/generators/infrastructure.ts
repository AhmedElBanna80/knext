...
export function generateInfrastructure(
    ...
    if (config.observability?.enabled) {
        ...
        // OTel Collector (opt-out with collector.enabled: false)
        if (config.observability.collector?.enabled !== false) {
            const collectorManifest = generateOtelCollectorManifest(
                config.name,
                config.observability,
            );
            const collectorPath = join(outputDir, "otel-collector.yaml");
            writeFileSync(collectorPath, collectorManifest);
            manifests.push(collectorPath);

            // Set OTLP endpoint for the app to reach the collector
...
/**
 * Generates an OpenTelemetry Collector deployment.
 * Receives OTLP from app pods and exports to Prometheus + logging.
 * Cloud-agnostic: no vendor-specific exporters.
 */
function generateOtelCollectorManifest(
    appName: string,
    config: ObservabilityConfig,
): string {
    const collectorImage =
        config.collector?.image ?? "otel/opentelemetry-collector-contrib";
    const collectorVersion = config.collector?.version ?? "0.115.0";
    const _scrapeInterval = config.prometheus?.scrapeInterval ?? "15s";

    return `# OpenTelemetry Collector for ${appName}
# Generated by kn-next CLI — cloud-agnostic, works on any Kubernetes cluster
---
apiVersion: v1
kind: Namespace
metadata:
  name: monitoring
  labels:
    app.kubernetes.io/part-of: kn-next-observability
---
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  config.yaml: |
    ...
    exporters:
      # Prometheus exporter — serves /metrics for Prometheus to scrape
      prometheus:
        endpoint: 0.0.0.0:8889
        namespace: kn_next
        resource_to_telemetry_conversion:
          enabled: true
      # Debug/logging exporter — writes spans to stdout
      debug:
        verbosity: basic
      # OTLP exporter — forward to any OTLP-compatible backend
      # (Jaeger, Tempo, Datadog, etc.) Uncomment and configure as needed:
      # otlp/backend:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: collector
          image: ${collectorImage}:${collectorVersion}
